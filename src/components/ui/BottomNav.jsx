...
const tabs = [
  { to: '/', label: 'Home', icon: 'M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z' },
...
export default function BottomNav() {
  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 bg-bg/90 backdrop-blur-xl border-t border-white/[0.06] flex justify-around py-2 pb-[max(8px,env(safe-area-inset-bottom))]">
      {tabs.map((tab) => (
        <NavLink key={tab.to} to={tab.to} end={tab.to === '/'} className="flex flex-col items-center gap-1 px-3 py-1">
          {({ isActive }) => (
            <>
              <motion.svg
